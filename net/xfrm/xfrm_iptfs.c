...
/**
 * iptfs_alloc_skb() - Allocate a new `skb` using a header template.
 * @tpl: the template to copy the new `skb`s header from.
 * @len: the linear length of the head data.
 *
 * A new `skb` is allocated and it's meta-data is initialized from `tpl`, the
 * head data is sized to `len` with the reserved space equal to the headroom
 * from `tpl` or XFRM_IPTFS_MIN_HEADROOM whichever is greater.
 */
...
#if 0
static bool iptfs_check_frags_ok(struct sk_buff *skb)
{
	if (!skb->head_frag)
		return false;
	if (!skb->data_len)
...
static int iptfs_input_ordered(struct xfrm_state *x, struct sk_buff *skb)
{
	...
	/* frags_ok = iptfs_check_frags_ok(skb); */
...
static void __vec_shift(struct xfrm_iptfs_data *xtfs, uint shift)
{
...
static void iptfs_consume_frags(struct sk_buff *to, struct sk_buff *from)
{
	...
	if (!skb_headlen(from))
		...
		struct page *page = virt_to_head_page(from->data);
		unsigned char *addr = (unsigned char *)page_address(page);

		skb_frag_fill_page_desc(&toi->frags[toi->nr_frags], page,
					from->data - addr, skb_headlen(from));
...
static void iptfs_output_queued(struct xfrm_state *x, struct sk_buff_head *list)
{
	...
	while ((skb = __skb_dequeue(list))) {
		...
		/*
		 * TODO: check to see if we can use page frags
		 * Use or reuse skb_gro_receive skb->head_frag case.
		 *
		 * Maybe walk the list to see if we will need more than
		 * MAX_SKB_FRAGS and if so just use the code below as things
		 * will take the slowpath anyway when we frag_list the page frag
		 * skbs.
		 */
