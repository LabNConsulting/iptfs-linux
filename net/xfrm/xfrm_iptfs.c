 ...
 */
static struct sk_buff *iptfs_alloc_skb(struct sk_buff *tpl, uint len, bool l3resv)
{
	struct sk_buff *skb;
	uint resv;
...
iptfs_pskb_extract_seq(uint skblen, struct skb_seq_state *st, uint off, int len)
{
 ...
 */
static uint __iptfs_iphlen(u8 *data)
{
...
static int iptfs_input_ordered(struct xfrm_state *x, struct sk_buff *skb)
{
	...
	struct iphdr *iph;
	struct net *net;
	uint remaining, first_iplen, iplen, iphlen, data, tail;
